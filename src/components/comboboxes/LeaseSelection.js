    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                ...
                >
                    {leaseId
                        ? leases?.find((lease) => lease.id === parseInt(leaseId))?.tenant 
                          ? `${leases.find((lease) => lease.id === parseInt(leaseId)).tenant.firstName} ${leases.find((lease) => lease.id === parseInt(leaseId)).tenant.lastName}`
                          : "Contrato sin inquilino"
                ...
                <Command>
                    ...
                    <div className="flex flex-row relative">
                        <CommandList className="w-full">
                            <CommandEmpty>No se encontró ningún contrato.</CommandEmpty>
                            <CommandGroup>
                                {leases?.map((lease) => (
                                    <CommandItem
                                        key={lease.id}
                        ...
                        <div
                            ...
                            <div>
                                ...
                                <p className="text-sm font-500 capitalize">
                                    {lease?.status ? lease?.status?.toLowerCase() :  "Sin estado"}
                                </p>
                            </div>
